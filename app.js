...
MongoClient.connect(mongoUri, function(err, db) {
    ...
    app.get('/bookshare', function(req, res) {
        db.collection('books').find({}).toArray(function(err, docs) {
            res.render('bookshare', {'books': docs} );
        });
    });

    app.get('/searchBook', function(req, res) {
        db.collection('books').find({'bookID': 1}).toArray(function(err, docs) {
            res.render('bookshare', {'books': docs} );
        });
    });

    app.get('/housing', function(req, res) {
        db.collection('housing').find({}).toArray(function(err, docs) {
            res.render('housing', {'housing': docs} );
        });
    });

    app.get('/searchHousing', function(req, res) {
        db.collection('housing').find({'location': "Fenway"}).toArray(function(err, docs) {
            res.render('housing', {'housing': docs} );
        });
    });
